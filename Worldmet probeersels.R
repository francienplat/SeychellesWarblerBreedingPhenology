...
# in excel data format naar dd mm jjjj gezet
...
breeders_clean <- breeders_only |>
  distinct(NestName, Status, .keep_all = TRUE)
...
readxl::read_xlsx("MaxTemperature_1997-2023.xlsx") -> MaxTemperature
##########################################FOUT###MOETMETMEDIAN LAYDATE#######
#climwin for 1997-2005
tempperiod1 <- breeders_wide |>
  filter(PeriodYear %in% 1997:2005)
resultsclimwintemp<- slidingwin(
  xvar = list(Temp = MaxTemperature$max_temp),
  cdate = MaxTemperature$day,
  bdate = tempperiod1$LayDateEst,
  baseline = lm(LayDateNum ~ 1, data = tempperiod1), 
  ...
  func = c("lin","quad"),
  cmissing = "method2")
resultsclimwintemp$combos
#save the results
save(resultsclimwintemp, file = "resultsclimwintemp.RData")
resultsclimwintemp[[2]]$BestModelData
plot(yvar~climate, data=resultsclimwintemp[[2]]$BestModelData, 
     xlab = "Temperature (°C)", ylab = "Lay Date (days since 1st January)")
######################FOUTFOUTFOUT!############################################
...
#so the mean temperature between 126 and 122 days before the median egg-laying
...
#new dataframe with mean temperature of the 126-122 days before the median egg-laying date
#1998-2005
...
# fit your model again if needed
model1 <- lm(MedianLayDate ~ MeanMaxTempWindow * TimePeriod, data = MaxTemperature)
...
pairs(em_trends)
#which version of emmeans package?
